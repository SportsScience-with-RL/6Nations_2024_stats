...
def eda_navigation(data_6nations, teams_img, games_scores, games_results, tbl_desc_full, tbl_desc_period):
    ...
    st.write("**- Durée :** durée des séquences en secondes")
    st.write("**- Phases :** nombre de phases (rucks) de la séquence")
    st.write("**- Ratio Rucks-Passe :** plus le ratio est élevé plus il y a de passes effectuées entre les rucks de la séquence")
    st.write("**- Avancée + :** somme des rucks et offloads après avoir cassé la ligne d'avantage de la séquence")
    st.write("**- Ratio Avancée :** un ratio élevé représente une plus grande capacité à jouer en dépassant la ligne d'avantage entre les rucks de la séquence")
    st.write("**- Progression Zones :** nombre de zones parcourues durant la séquence par rapport à la zone de début et zone de fin")
    st.write('')
